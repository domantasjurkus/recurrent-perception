...
def test_video(model, test_loader, n_classes, TEST_LOSS_MULTIPLY, device="cpu"):
    model.eval()
    correct = 0
    total = 0
    ...
    with torch.no_grad():
        for i, data in enumerate(test_loader):
            ...
            batchsize, timesteps, c, h, w = video.shape
            video = video.view(batchsize*timesteps, c, h, w)
            ...
            _, per_frame_index = torch.max(outputs.data, 1)
            predicted_index, _ = torch.mode(per_frame_index, dim=0)
            ...
            batch_size = 1
            total += batch_size
            correct_vector = (predicted_index == targets)
