...
def init_hidden_and_cell(hidden_size=128, device="cuda:0"):
    ...
    h0 = torch.randn(1, 1, hidden_size, device=device)
    c0 = torch.randn(1, 1, hidden_size, device=device)
    return (h0, c0)
    ...
    for epoch in range(1, epochs+1):
        ...
        for i, data in enumerate(train_loader):
            ...
            for video in batch:
                ...
                for frame in video:
                    # frame.shape = (channels=1, h, w)
                    # a single timestep will be processed at a time
                    frame.unsqueeze_(0)

                    outputs, hc = model(frame, hc)
