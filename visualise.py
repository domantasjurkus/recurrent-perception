...
model.load_state_dict(torch.load("saved_models/cifarbased_masked_50epochs.pt"))
...
image = cv2.imread("../project-data/xtion1/masked/pant/01_01_0050.png", cv2.IMREAD_GRAYSCALE)
# image = cv2.resize(image, (240, 320))

grad_cam = GradCam(model=model, target_layer_names=["5"], use_cuda=False)
...
for i in range(40, 51):
    image = cv2.imread("../project-data/xtion1/masked/pant/01_01_00%d.png" % i, cv2.IMREAD_GRAYSCALE)
    image = torch.tensor(image).float()
    image.unsqueeze_(0)
    image.unsqueeze_(0)
    grad_cam(image, None)